# Target Brief: Artifact Emphasis Checker
...
Create a user-facing `Artifact Emphasis Checker` agent.

Purpose:
Help the user verify whether one explicit emphasized phrase appears in one explicit workspace file.
...
- prefer exact file-target resolution and block instead of guessing when the file or emphasized phrase target is ambiguous
- do not silently widen from one named file to repo-wide search
- do not block initial creation on optional normalization between `*italic*` and `**bold**` forms or report-format refinements