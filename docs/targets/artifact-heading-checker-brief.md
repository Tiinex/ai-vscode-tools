# Target Brief: Artifact Heading Checker
...
Create a user-facing `Artifact Heading Checker` agent.

Purpose:
Help the user verify whether one explicit Markdown heading appears in one explicit workspace file.

Expected high-level behavior:
- inspect only one explicit workspace file named by the user
- check one explicit heading at a time
- return a concise verdict such as `found`, `not-found`, or `ambiguous`
- include the matching heading line and a small nearby excerpt only when it is directly recoverable from the named file
...
- prefer exact file-target resolution and block instead of guessing when the file or heading is ambiguous
- do not silently widen from one named file to repo-wide search
- do not block initial creation on optional heading-normalization or report-format refinements