# Target Brief: Artifact Inline Code Checker
...
Create a user-facing `Artifact Inline Code Checker` agent.

Purpose:
Help the user verify whether one explicit inline code fragment appears in one explicit workspace file.
...
- prefer exact file-target resolution and block instead of guessing when the file or inline-code target is ambiguous
- do not silently widen from one named file to repo-wide search
- do not block initial creation on optional whitespace-normalization or report-format refinements