# Target Brief: Artifact Code Comment Checker
...
Create a user-facing `Artifact Code Comment Checker` agent.

Purpose:
Help the user verify whether one explicit code comment appears in one explicit workspace file.

Expected high-level behavior:
- inspect only one explicit workspace file named by the user
- check one explicit code comment at a time
...
- prefer exact file-target resolution and block instead of guessing when the file or code-comment target is ambiguous
- do not silently widen from one named file to repo-wide search
- do not block initial creation on optional normalization across comment syntaxes such as `#`, `//`, or `/* */`