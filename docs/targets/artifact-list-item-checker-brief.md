# Target Brief: Artifact List Item Checker
...
Create a user-facing `Artifact List Item Checker` agent.

Purpose:
Help the user verify whether one explicit list item appears in one explicit workspace file.

Expected high-level behavior:
- inspect only one explicit workspace file named by the user
- check one explicit list item at a time
...
- prefer exact file-target resolution and block instead of guessing when the file or list item is ambiguous
- do not silently widen from one named file to repo-wide search
- do not block initial creation on optional normalization of bullet markers or report-format refinements