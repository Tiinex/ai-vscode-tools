  ...
  try {
    ...
    try {
      ...
      assert(
        ...
        `Blocking focused-send did not diagnose the unfinished assistant tool turn after timeout. Got: ${unfinishedAgentTurnResult.reason}`
      );
    } finally {
    ...
    try {
      ...
      await fs.writeFile(settledFocusedSessionFile, `${JSON.stringify({
        ...
        v: [{
          ...
          result: { details: 'GPT-5.4 â€¢ 1x' },
          message: { text: 'hello' },
          response: [{ value: 'Steg 3 bekrÃ¤ftat.' }],
      ...
      await fs.writeFile(settledFocusedTranscriptFile, `${[
        ...
        JSON.stringify({ type: 'assistant.message', data: { content: 'Steg 3 bekrÃ¤ftat.', toolRequests: [] }, timestamp: '2026-05-09T07:20:20.784Z' }),
      ...
      JSON.stringify({
        ...
        v: {
          ...
          requests: [{
            ...
            result: { details: 'GPT-5 mini â€¢ 0x' },
...
async function runPendingRequestHeuristicChecks() {
  ...
  await fs.writeFile(laggingSessionFile, [
    ...
    JSON.stringify({ kind: 1, k: ["requests", 0, "result"], v: { metadata: {}, value: "GPT-5 mini â€¢ 0x" } })
  ].join("\n") + "\n", "utf-8");
  await fs.writeFile(laggingTranscriptFile, [
